...
public class Practice07Controlller {
	// 1. localhost:8080/practice07/listTest ?type=str 경로로 접근시
	// 2. localhost:8080/practice07/listTest ?type=member 경로로 접근시
	/*
	 * @GetMapping("/practice07/listTest/member") public String
	 * listTestMember(@RequestParam String type,Model model) { // str 반복 케이스 //1) 그냥
	 * jsp 내에서 고정 텍스트 반복
	 * 
	 ...
	 * 
	 * // 화면 처리 케이스 // 1) type으로 아예 화면 자체를 구분
	 * 
	 ...
	 * 
	 * return "practice/practice07/str"; } else { //member
	 * 
	 ...
	 * Member("아이디2","비번2","이름2")); // memberList.add(new
	 * Member("아이디3","비번3","이름3")); // memberList.add(new
	 * Member("아이디4","비번4","이름4")); // memberList.add(new
	 * Member("아이디5","비번5","이름5"));
	 * 
	 ...
	 * 
	 * // 2) 같은 화면에서 type에 따라 동적으로 구분 model.addAttribute("type",type);
	 * 
	 * //각 type 파라미터값에 맞춰서 type에 따라 동적으로 구분 List<Member> memberList = new
	 * ArrayList<Member>();
	 * 
	 ...
	 * 
	 * @GetMapping("/practice07/listTest/str") public String listTestStr(Model
	 * model) { // str 반복 케이스 //1) 그냥 jsp 내에서 고정 텍스트 반복
	 * 
	 * //2) 전달할 스트링(메시지) 전달하고 그걸 반복
	 * 
	 * //3) 전달할 메시지를 리스트 형태로 전달 반복
	 ...
	 * return "practice/practice07/member"; }
	 * 
	 * // 2) 같은 화면에서 type에 따라 동적으로 구분 String type = "str";
	...
	public String listTest(@RequestParam String type, Model model) {
		...
		// 각 type 파라미터값에 맞춰서 type에 따라 동적으로 구분
		if (type.equals("str")) {
