...
public class practice08Controller {
	...
	public String member(@RequestParam String auth, Model model) {
		...
		if (!(auth.equals("basic") || auth.equals("manager") || auth.equals("admin"))) {
			// auth == "basic" "manager" "admin" 이 세 개가 아닌경우
			// 파라미터가 잘못된 경우
	 ...
	 * public class practice08Controller {
	 * 
	 * @RequestMapping("/member") public String memberAuth(@RequestParam String
	 * auth,Model model) { List<Member> memberList = new ArrayList<Member>();
	 * 
	 * memberList.add(new Member("user1", "pass123", "홍길동", "basic"));
	 * memberList.add(new Member("user2", "pass456", "김철수", "manager"));
	 * memberList.add(new Member("user3", "pass789", "이영희", "basic"));
	 ...
	 * memberList.add(new Member("user10", "passabc", "한영희", "manager"));
	 * 
	 * if(auth.equals("basic")) { List<Member> basicList = new ArrayList<Member>();
	 * for(Member m:memberList) { if(m.getType().equals("basic")) {
	 * basicList.add(m); } } model.addAttribute("basicList",basicList); } else
	 * if(auth.equals("manager")) { List<Member> managerList = new
	 * ArrayList<Member>(); for(Member m:memberList) {
	 * if(m.getType().equals("manager")) { managerList.add(m); } }
	 * model.addAttribute("managerList",managerList); } else {
	 * model.addAttribute("memberList",memberList); } return
	 * "practice/practice08/memberView"; }
	 */
}
